        ...
        <div className="flex items-center gap-2">
          ...
          <div
            className={`w-2 h-2 rounded-full ${
              networkInfo.chainId ? "bg-green-500" : "bg-red-500"
            }`}
          />
          <span className="text-sm font-medium text-slate-200">
            {networkInfo.name}
    ...
    <div className="min-h-screen bg-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={() => open()}
                className="flex items-center px-4 py-2 rounded-lg bg-[#845DDE] hover:bg-blue-600 text-white transition-colors"
              >
                {address ? (
        ...
        <div className="md:hidden border-t border-slate-700">
          <div className="flex justify-center gap-4 py-3">
            <Link
              to="/"
              className={`nav-link ${
                isActiveRoute("/") ? "text-[#845DDE]" : "text-slate-300"
              }`}
            >
              Home
            </Link>
            <Link
              to="/create"
              className={`nav-link ${
                isActiveRoute("/create") ? "text-[#845DDE]" : "text-slate-300"
              }`}
            >
              Create
            </Link>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-slate-800/90 backdrop-blur-sm border-t border-slate-700">
        <div className="max-w-7xl mx-auto px-4 py-4 text-center text-slate-400 text-sm">
          MoonTerminal © {new Date().getFullYear()} All rights reserved.
        </div>
      </footer>
    </div>
  );
